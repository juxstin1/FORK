...
**Brain** (`app/src/server/index.ts`) — An MCP server using `@modelcontextprotocol/sdk` over stdio. Exposes 11 tools to Claude including pipeline stages, file operations, and external LLM delegation. The server resolves `projectDir` as `path.resolve(process.cwd(), "..")` — it assumes it runs from `app/`.
...
- **design.ts** — Takes a DesignSpec JSON → saves to `.rork/design.json` (stub — needs screen generator, flow mapper)
...
**Known gap:** `run_persona_stage` is imported but not registered as a tool. Persona generation currently requires direct code invocation.
...
**Note:** Stage files in `prompts/stages/` export prompt constants, but the REGISTRY is defined inline in `index.ts`. The stage file exports are currently unused.
...
| `/fork:design` | DESIGN | Convert requirements → screens + flows |
| `/fork:build` | BUILD | Generate production Expo code |
...
| `/fork:dev` | Utility | Start Expo dev server |
...
- Persona stage sits between IDEA and DESIGN (not shown in original 6-stage shorthand)
